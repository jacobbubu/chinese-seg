  ...
  data.split(/\r?\n/).forEach (jsonWord) ->
    ...
    if jsonWord.length > 0
      ...
      if oneWord?
        ...
        if len > 0
          dict.byWord[oneWord.word] = {props: oneWord.props, freq: oneWord.freq}
          dict.byLen[''+len] ?= {}
          dict.byLen[''+len][oneWord.word] = dict.byWord[oneWord.word]
  dict
...
module.exports = (options) ->
  ...
  options.skipProps = unique Object.keys(allProps).concat(options.skipList ? [])
  ...
  # fine best matching phrase
  filterWord = (ret, prevParam, text) ->
  ...
  matchWord = (text, curr, prevWord) ->
    ...
    while (curr < text.length)
      for len, wordList of byLen
        w = text.substr curr, Number len
        ret.push { w: w, curr: curr, freq: wordList[w].freq} if wordList[w]?
      curr++
